function handleMessage(messageDataObject){
    // insert handler here
}
(function(){
  setTimeout(()=>{
    ...
    for (io = 0;io !== document.getElementsByClassName("content").length;io++){
      ...
      messageData.send = function(message){document.getElementById("input").value = message;document.getElementById("sayit-button").click();}
      messageData.reply = function(message){document.getElementById("input").value = ":" + messageData.messageId + " " + message;document.getElementById("sayit-button").click();}
      // moderator check
